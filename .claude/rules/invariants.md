...
## Shell Allowlist
NL seeding: `is_permitted_shell_command()` at `src/runtime/investigation/prompt_analysis.rs:269` — matches only `"cargo"`. Enforced in `TurnContext` construction in `engine.rs`; suppressed entirely on `GitReadOnly` turns. Unchanged by Phase 47.
...
- **Tier 2 (FsMutation)**: `mkdir`, `rmdir`, `cp`, `mv` — approval-gated, `reversible: true`.
