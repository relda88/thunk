...
fn fabrication_triggers_correction_then_normal_answer() {
    ...
    let mut rt = make_runtime_in(vec![fabricated, "Let me answer directly."], dir.path());
...
fn malformed_open_tag_triggers_correction_and_retries() {
    ...
    let mut rt = make_runtime_in(vec![malformed, "Done."], dir.path());
...
fn non_identifier_question_allows_direct_unchanged() {
    ...
    let dir = TempDir::new().unwrap();
    let mut rt = make_runtime_in(vec!["I'm doing well, thanks!"], dir.path());
...
fn approve_path_does_not_trigger_r1() {
    ...
    let mut rt = make_runtime_in(vec!["I couldn't apply the edit_file change."], dir.path());
